...
class DetectionService:
    ...
    def detect_all_channel_edges(self, flat_image_data):
        ...
        points_dict = self._combine_edges_to_points(
    ...
    def _detect_horizontal_edges(self, image_data):
        ...
        # Test Step 4: 30x0 Sobel filter (30 Pooling)
        derivatives = Computation.compute_sobel_filter_at_positions(
            image_data, 
            axis=0, 
            positions=y_positions)
        ...
        detected_lines = self._detect_edge_positions_x(
            derivatives, y_positions)
    ...
    def _detect_vertical_edges(self, image_data, be_points):
        ...
        detected_edges = self._detect_edge_positions_y(
            derivatives, x_positions)
    ...
    def _detect_field_bounds(self, mean_derivative):
        ...
        last_peak = next(
            (i for i, v in reversed(list(enumerate(mean_derivative)))
             if abs(v) > threshold),
            None
        )
    ...
    def _detect_edge_positions_x(self, derivatives, y_positions):
        ...
        for y in y_positions:
            ...
            edges_x = Computation.top_n_local_maxima(
                # Negative because we want minima of derivative
                -derivatives[y],
                18  # 9 channels × 2 edges
            )
            # Sort by X position and pair with Y
            line_points = sorted([(xi, y)
