#!/usr/bin/env python3
"""
Vérification de la réussite du pipeline Python MSDP.

Contrôle que les RÉSULTATS du pipeline Python (ms1.py + ms2.py) sont bons,
et pas seulement que le pipeline s'est exécuté sans erreur. Il analyse le
fichier ``ACDF2.lis`` produit par ms2.py (en-têtes de coins des 9 canaux) et
s'assure que la géométrie détectée est physiquement cohérente.

Critères vérifiés (mêmes critères internes que `check_fortran_pipeline.py`, sans
comparaison au pipeline Fortran) :
  1. Présence et format du fichier : 9 lignes (9 canaux) × 8 valeurs numériques.
  2. Aucune valeur nulle (0.00) et aucun NaN — un 0 signifie un canal non détecté.
  3. Ordre des canaux : les centres X croissent strictement de canal en canal.
  4. Régularité des centres X : l'écart inter-canaux est stable (tolérance).
  5. Largeur de canal cohérente : écart-type / moyenne < tolérance.
  6. Bornes physiques : chaque coordonnée est dans l'image CCD permutée.

Sorties : rapport lisible + code de retour (0 = succès, 1 = échec).

Usage :
    python3 check_python_pipeline.py [ACDF2.lis [ms.yml]]
      - ACDF2.lis : fichier à contrôler (défaut: data/output/ACDF2.lis)
      - ms.yml    : paramètres Python, affiché en tête de rapport (optionnel)
"""

import sys
import math
from pathlib import Path

# ------- Configuration -------
DEFAULT_ACDF2 = Path("data/output/ACDF2.lis")
DEFAULT_MS_YML = Path("src/python/ms.yml")
NM_EXPECTED_DEF = 9     # nombre de canaux par défaut
COL_PER_ROW = 8          # A_X, C_X, D_X, F_X, A_Y, C_Y, D_Y, F_Y
# Bornes physiques (image permutée du pipeline newgeom) — défauts
IM_PIX_DEF = 1536
JM_PIX_DEF = 1536
MAX_ABS = 2500.0
# Tolérances de régularité
CTR_X_TOL_FRAC = 0.25    # écart-type relatif des pas de centres X
WIDTH_TOL_FRAC = 0.35    # écart-type relatif des largeurs de canal
# ~7e critère — alignement des bords haut/bas sur des droites parallèles (2026-09-06)
EDGE_Y_MAX_DEV = 3.0     # px max d'écart d'un point haut/bas à sa droite (saines: <1px)
EDGE_Y_PAR_TOL = 0.10    # px/canal : |pente_hi - pente_lo| max (parallélisme)


class Config:
    """Paramètres de validation, chargés depuis ms.yml (ou valeurs par défaut)."""
    def __init__(self, nm, im_pix, jm_pix, ms_yml_path=None):
        self.nm = nm
        self.im_pix = im_pix
        self.jm_pix = jm_pix
        self.ms_yml_path = ms_yml_path


def load_config_python(ms_yml_path: Path | None) -> Config:
    """Charge nm, im, jm depuis ms.yml (champs YAML)."""
    nm, im, jm = NM_EXPECTED_DEF, IM_PIX_DEF, JM_PIX_DEF
    if ms_yml_path and Path(ms_yml_path).exists():
        try:
            import yaml
            with open(ms_yml_path) as f:
                data = yaml.safe_load(f) or {}
            nm = int(data.get('nm', nm))
            im = int(data.get('im', im))
            jm = int(data.get('jm', jm))
        except Exception:
            pass
    return Config(nm, im, jm, ms_yml_path)


def parse_acdf2(path: Path) -> list[list[float]]:
    """Lit le fichier et retourne une liste de 9 listes de 8 floats."""
    rows = []
    with open(path) as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            vals = []
            for p in line.split():
                try:
                    vals.append(float(p))
                except ValueError:
                    raise ValueError(f"valeur non numérique « {p} » dans {path}")
            rows.append(vals)
    return rows


def stddev(xs: list[float]) -> float:
    n = len(xs)
    if n < 2:
        return 0.0
    m = sum(xs) / n
    return math.sqrt(sum((x - m) ** 2 for x in xs) / (n - 1))


def _linreg_dev(points: list[tuple[float, float]]) -> tuple[float, float]:
    """Régression linéaire Y = a·X + b sur des points (X,Y).

    Retourne (pente a, déviation max |résidu| en px). Utilisé pour vérifier
    que les bords haut/bas des canaux suivent une droite (critère 7).
    """
    if len(points) < 2:
        return 0.0, 0.0
    xs = [p[0] for p in points]
    ys = [p[1] for p in points]
    n = len(xs)
    mx = sum(xs) / n
    my = sum(ys) / n
    num = sum((x - mx) * (y - my) for x, y in zip(xs, ys))
    den = sum((x - mx) ** 2 for x in xs)
    slope = num / den if den != 0 else 0.0
    intercept = my - slope * mx
    max_dev = max(abs(y - (slope * x + intercept)) for x, y in points)
    return slope, max_dev


def run_checks(path: Path, config: Config | None = None) -> tuple[bool, list[str]]:
    ok = True
    if config is None:
        config = load_config_python(DEFAULT_MS_YML)
    nm_exp = config.nm
    im_pix, jm_pix = config.im_pix, config.jm_pix
    msgs = []
    add = msgs.append

    # -- 1. Format ------------------------------------------------
    if not path.exists():
        return False, [f"❌ ACDF2.lis introuvable : {path}"]
    try:
        rows = parse_acdf2(path)
    except Exception as e:
        return False, [f"❌ Lecture impossible de {path} : {e}"]

    if len(rows) != nm_exp:
        ok = False
        add(f"❌ Nombre de canaux : {len(rows)} (attendu {nm_exp})")
    else:
        add(f"✔  {nm_exp} canaux")

    for i, r in enumerate(rows):
        if len(r) != COL_PER_ROW:
            ok = False
            add(f"❌ Canal {i+1}: {len(r)} valeurs (attendu {COL_PER_ROW})")

    # -- 2. Valeurs nulles / NaN / bornes -------------------------
    bad_zero = bad_nan = bad_bound = 0
    for i, r in enumerate(rows[:nm_exp]):
        if len(r) != COL_PER_ROW:
            continue
        x = r[0:4]
        y = r[4:8]
        for v in x + y:
            if math.isnan(v) or math.isinf(v):
                bad_nan += 1
            elif abs(v) < 1e-6:
                bad_zero += 1
            elif abs(v) > MAX_ABS:
                bad_bound += 1
        for v in x:
            if not (0 <= v <= im_pix):
                bad_bound += 1
        for v in y:
            if not (0 <= v <= jm_pix):
                bad_bound += 1
    if bad_zero:
        ok = False
        add(f"❌ {bad_zero} valeur(s) nulle(s) (canal non détecté)")
    if bad_nan:
        ok = False
        add(f"❌ {bad_nan} valeur(s) NaN/infini (overflow)")
    if bad_bound:
        ok = False
        add(f"❌ {bad_bound} valeur(s) hors bornes physiques")

    # -- 3. Centres X croissants et réguliers ---------------------
    if len(rows) >= nm_exp and all(len(r) == COL_PER_ROW for r in rows[:nm_exp]):
        centers = [(r[0] + r[1] + r[2] + r[3]) / 4.0 for r in rows[:nm_exp]]
        increases = [centers[i+1] > centers[i] for i in range(len(centers)-1)]
        if not all(increases):
            ok = False
            add(f"❌ Centres X non croissants ({sum(1 for i in increases if not i)} décroissance(s))")
        else:
            add("✔  Centres X strictement croissants")
        steps = [centers[i+1] - centers[i] for i in range(len(centers)-1)]
        if steps:
            mean_step = sum(steps) / len(steps)
            if mean_step > 0:
                rel = stddev(steps) / mean_step
                if rel > CTR_X_TOL_FRAC:
                    ok = False
                    add(f"❌ Pas inter-canaux irrégulier (rel.stdev={rel:.2%})")
                else:
                    add(f"✔  Pas inter-canaux régulier ({mean_step:.1f} px/ canal, σ={rel:.2%})")
            else:
                ok = False
                add("❌ Pas inter-canaux moyen non positif")

        # -- 4. Largeur de canal cohérente ------------------------
        widths = []
        for r in rows[:nm_exp]:
            w1 = r[2] - r[0]
            w2 = r[3] - r[1]
            if w1 > 0:
                widths.append(w1)
            if w2 > 0:
                widths.append(w2)
        if widths:
            mean_w = sum(widths) / len(widths)
            if mean_w > 0:
                rel = stddev(widths) / mean_w
                if rel > WIDTH_TOL_FRAC:
                    ok = False
                    add(f"❌ Largeurs de canaux incohérentes (rel.stdev={rel:.2%})")
                else:
                    add(f"✔  Largeurs cohérentes ({mean_w:.1f} px, σ={rel:.2%})")
            else:
                ok = False
                add("❌ Largeur de canal moyenne non positive")

    # -- 7. Alignement des bords haut/bas sur des droites parallèles -----
    # (2026-09-06) voir check_fortran_pipeline.py — mêmes critères internes.
    if len(rows) >= 2:
        n = len(rows)
        # abscisse = centre X du canal (milieu de la largeur)
        xs = [(r[0] + r[2]) / 2.0 for r in rows]   # (AX+DX)/2
        # points haut : CY, FY ; points bas : AY, DY
        hi = [(xs[i], cy) for i, r in enumerate(rows) for cy in (r[5], r[7])]
        lo = [(xs[i], ay) for i, r in enumerate(rows) for ay in (r[4], r[6])]

        slope_hi, max_dev_hi = _linreg_dev(hi)
        slope_lo, max_dev_lo = _linreg_dev(lo)
        par_dev = abs(slope_hi - slope_lo)

        fail_dev = max(max_dev_hi, max_dev_lo) > EDGE_Y_MAX_DEV
        fail_par = par_dev > EDGE_Y_PAR_TOL
        if fail_dev or fail_par:
            ok = False
            if fail_dev:
                add(f"❌ Bords haut/bas hors droite (dév. max {max(max_dev_hi, max_dev_lo):.2f} px "
                    f"> tol {EDGE_Y_MAX_DEV} px)")
            if fail_par:
                add(f"❌ Droites haut/bas non parallèles (Δpente {par_dev:.3f} > tol {EDGE_Y_PAR_TOL})")
        else:
            add(f"✔  Bords haut/bas alignés (dév. max {max(max_dev_hi, max_dev_lo):.2f} px, "
                f"Δpente {par_dev:.3f})")

    # -- Bilan ------------------------------------------------
    add("")
    add("SUCCÈS : la géométrie du pipeline Python est cohérente." if ok
        else "ÉCHEC : des anomalies ont été détectées dans la géométrie Python.")
    return ok, msgs


def main() -> int:
    acdf2_path = Path(sys.argv[1]) if len(sys.argv) > 1 else DEFAULT_ACDF2
    ms_yml = sys.argv[2] if len(sys.argv) > 2 else str(DEFAULT_MS_YML)

    config = load_config_python(ms_yml)

    print("=" * 60)
    print("  Vérification du pipeline Python MSDP")
    print(f"  Fichier : {acdf2_path}")
    if ms_yml:
        print(f"  Paramètres : {ms_yml}")
    print(f"  Config : nm={config.nm}, im={config.im_pix}, jm={config.jm_pix}")
    print("=" * 60)

    ok, msgs = run_checks(acdf2_path, config)
    for m in msgs:
        print(f"  {m}")
    print("=" * 60)
    return 0 if ok else 1


if __name__ == "__main__":
    sys.exit(main())