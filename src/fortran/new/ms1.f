...
c               geom                  calib
      ...
      dimension sobstot(2000,200,24,10)
c***********************************
...
c     1      form='unformatted',status='new')
      ...
      call system('rm miv.lis')
      open(unit=4,file='miv.lis',status='new')
      print *,'call readpar'
      call readpar
...
c=============================================Calibration
      ...
      write(3,*)icalib
      if(icalib.eq.0)stop
      write(3,*)' entree channels'
      call channels(xr,yr, ima, imc,jmc, cymx,imd,jmd,nm)
c                   in     IN            OUT
c----------------------------------cal.ps
      print *,' call calib: imd,jmd,nm ',imd,jmd,nm
      call calib(cymx,imd,jmd,nm,cliss,jtr,cal,xr,yr)
c                IN      
      write(3,*)' fin calib '
c---------------------------------          files m*-> c*  d*
c      close(unit=31)
...
C                   CHANGEMENT D'ECHELLE X
 ...
 7      continue
      DO10 M=1,NT
        DO9 K=M,NT
        C(K,M)=C(K,M)+F(K)*F(M)*P(L)
 9      continue
      C(NTP,M)=C(NTP,M)+F(M)*P(L)*Y(L)
 10   continue
 12   continue
 ...
 20   continue
      ...
      PIVOT=C(I,I)
        DO27 M=1,NT                ! M
        C(M+1,I)=C(M+1,I)/PIVOT
 27     continue
      DO29 K=1,NT                  !K
      IF(I.EQ.K)GOTO29
      PIVOT=C(I,K)
        DO28 M=1,NT                !M
        C(M+1,K)=C(M+1,K)-C(M+1,I)*PIVOT
 28   continue
 29   continue
 30   CONTINUE
C
      COEF(1)=C(NTP,1)
      DO40 I=2,NT
      COEF(I)=C(NTP,I)/(1000.**(I-1))
 40   continue
c      write(3,*)' DPMCAR COEF'
c      write(3,*)(COEF(I),I=1,9)
      RETURN
      END
c======================================================
