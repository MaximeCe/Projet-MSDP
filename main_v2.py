...
def detect_edges_following_x(flat_path, dark_path):
    """
    ...
    if first_peak is None or last_peak is None:
        raise ValueError("Impossible de trouver les pics dans la dérivée moyenne.")

    y_positions = [first_peak + 21, first_peak + (last_peak - first_peak)//2, last_peak - 20]
    ...
    for idx, (y, derivative) in enumerate(derivatives.items()):
        ...
        x = top_n_local_maxima(np.abs(derivative), 18)
        filtered_points = [(xi, y) for xi in x]
...
def detect_edges_following_y(flat_path, dark_path, be_list: list[tuple]):
    """
    ...
    be_list= sorted(be_list, key=lambda x: x[0])  # trier par la première valeur du tuple
    ...
    b_indices = [i for i in range(len(be_list)) if i %
                 2 == 0]  # indices de début
    e_indices = [i for i in range(len(be_list)) if i %
                 2 != 0]  # indices de fin
    ...
    for idx, x in enumerate(x_positions):
        ...
        filtered_points = top_n_local_maxima(np.abs(derivatives[x]), 2)
        
        # ranger les points selon x puis y
        
        y = top_n_local_maxima(np.abs(derivatives[x]), 2)
        column = sorted([(x, yi) for yi in y], key=lambda point: (point[0], point[1]))
...
def compute_lines_coefficients(detected_columns):
    ...
    Args:
        detected_columns (list): A list of 18 sublists, where each sublist contains two points representing a detected column.
    Returns:
...
def main():
    ...
    detected_points_1, detected_points_2, detected_points_3 = detect_edges_following_x(
        flat_path, dark_path)
    ...
    detected_points_h = detect_edges_following_y(
        flat_path, dark_path, detected_points_2)
    if not detected_points_h:
        print("Erreur lors de la détection des bords.")
        return

    # Affichage des points détectés sur l'image flat
    display_detected_points(flat, detected_points_1, detected_points_2, detected_points_3, detected_points_h)
